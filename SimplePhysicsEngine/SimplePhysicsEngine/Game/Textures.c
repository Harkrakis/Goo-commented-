...
Textures *Textures_New(Renderer *renderer)
{
    ...
    if (!textures->m_ground)
    {
     ...
     textures->m_arrow = IMG_LoadTexture(renderer->m_rendererSDL, "../Assets/Arrow.png");
    if (!textures->m_body)
    {
        printf("ERROR - IMG_LoadTexture\n");
        printf("      - %s\n", SDL_GetError());
    ...
    textures->m_arrow1 = IMG_LoadTexture(renderer->m_rendererSDL, "../Assets/Arrow1.png");
    if (!textures->m_arrow1)
    {
        printf("ERROR - IMG_LoadTexture\n");
        printf("      - %s\n", SDL_GetError());
        goto ERROR_LABEL;
    ...
    textures->m_moon = IMG_LoadTexture(renderer->m_rendererSDL, "../Assets/moon.png");
    if (!textures->m_moon)
    {
        printf("ERROR - IMG_LoadTexture\n");
        printf("      - %s\n", SDL_GetError());
     ...
     textures->m_nograv = IMG_LoadTexture(renderer->m_rendererSDL, "../Assets/NoGrav.png");
    if (!textures->m_nograv)
    {
        printf("ERROR - IMG_LoadTexture\n");
        printf("      - %s\n", SDL_GetError());
        goto ERROR_LABEL;
...
void Textures_Free(Textures *textures)
{
    ...
    if (textures->m_arrow)
    {
        SDL_DestroyTexture(textures->m_arrow);
    }
    for (int i = 0; i < LAYER_COUNT; ++i)
    {
        if (textures->m_layers[i])
